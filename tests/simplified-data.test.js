...
describe('SimplifiedDataProvider', () => {
  ...
  describe('getQuote', () => {
    it('should get quote from Yahoo Finance first', async () => {
      const mockYahooResponse = {
        chart: {
          result: [{
            meta: { regularMarketPrice: 150.50, previousClose: 149.00 },
            timestamp: [1640995200],
            indicators: { quote: [{ close: [150.50] }] }
          }]
        }
      };
    ...
    it('should throw error when all sources fail', async () => {
      ...
      await expect(provider.getQuote('AAPL')).rejects.toThrow('Unable to fetch quote for AAPL');
    });
  });
      ...
      const mockYahooResponse = {
        chart: {
          result: [{
            ...
            indicators: { 
              quote: [{
                close: [150.50, 151.00, 149.50],
                high: [151.00, 152.00, 150.00],
                low: [149.00, 150.00, 148.00],
  ...
  describe('getVolatilityAnalysis', () => {
    it('should perform complete volatility analysis with real data', async () => {
      ...
      const prices = Array.from({ length: 60 }, (_, i) => 150 + Math.sin(i * 0.1) * 5);
      const mockHistoricalResponse = {
        chart: {
          result: [{
            timestamp: Array.from({ length: 60 }, (_, i) => 1640995200 + i * 86400),
            indicators: { 
              quote: [{
                close: prices,
      ...
      fetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(mockQuoteResponse) })
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(mockHistoricalResponse) });
    ...
    it('should handle mixed success/failure scenarios', async () => {
      ...
      fetch
        .mockResolvedValueOnce({ ok: true, json: () => Promise.resolve({
          chart: { result: [{ meta: { regularMarketPrice: 150.50, previousClose: 149.00 } }] }
        })})
